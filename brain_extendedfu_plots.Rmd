...
### Baseline and 4 Years Only, All Patients
This plot shows four-year progression for all BRAIN-ICU patients who remained in the study throughout their hospital stay.

```{r sankey_overall_base4}
sankeyDataBase4 <- createSankeyData(keepNodes = c(1, 5))
sankeyPlotBase4 <- createSankeyPlot(sankeyDataBase4)

```

```{r print_sankey_base4, results = 'asis'}
## For creating separate file
# print(sankeyPlotBase4, file = 'brain_phase3_sankey_base4.html')
print(sankeyPlotBase4, tag = 'chart')
...
### Baseline and 6 Years Only, All Patients
This plot shows the progression of all patients who remained in the study throughout their hospital
stay at 6-year followup.

```{r sankey_overall_base6}
sankeyDataBase6 <- createSankeyData(keepNodes = c(1, 6))
sankeyPlotBase6 <- createSankeyPlot(sankeyDataBase6)

```

```{r print_sankey_base6, results = 'asis'}
## For creating separate file
# print(sankeyPlotBase6, file = 'brain_phase3_sankey_base6.html')
print(sankeyPlotBase6, tag = 'chart')

```

### Baseline and 6 Years Only, Patients Assessed at Each Point
This plot shows progression of all patients who were assessed at baseline (IQCODE) *and* 6-year followup (RBANS global/Trails B).

```{r sankey_assessed_base6}
brainStatusAssessedBase6 <- subset(brainStatus, id %in% intersect(assessed_base, assessed_6y))

sankeyDataAssessedBase6 <- createSankeyData(df = brainStatusAssessedBase6, keepNodes = c(1, 6))
sankeyPlotAssessedBase6 <- createSankeyPlot(sankeyDataAssessedBase6)

```

```{r print_sankey_assessed_base6, results = 'asis'}
## For creating separate file
# print(sankeyPlotAssessedBase6, file = 'brain_phase3_sankey_assessed_base6.html')
print(sankeyPlotAssessedBase6, tag = 'chart')
...
# Raw RBANS Global Scores
The following plot(s) show RBANS global scores at 3m, 12m and 4 years by amount of delirium
experienced in the hospital: none, lower 50% among exposed, and upper 50% among exposed.

```{r spaghetti_datamgmt}
## Merge days of delirium with RBANS global scores
rbans.data <- subset(brain.fu,
                     !is.na(rbans.global.score), select = c(id, fu.period, rbans.global.score)) %>%
  filter(fu.period != '6 year') %>%
  left_join(dplyr::select(brain.oneobs, id, del.s.imp)) %>%
  mutate(xval = ifelse(fu.period == '3 Month', 1,
                ifelse(fu.period == '12 Month', 12 / 3,
                ifelse(fu.period == '4 year', 48 / 3, NA))))

## What's the delirium cutoff? Median of delirium duration among those who ever experienced it
del.cutoff <- median(unique(subset(rbans.data, del.s.imp > 0, select = c(id, del.s.imp)))$del.s.imp,
                     na.rm = TRUE)

## Add delirium category to data set
rbans.data$del.cat <- with(rbans.data, factor(ifelse(del.s.imp == 0, 1,
                                              ifelse(del.s.imp < del.cutoff, 2,
                                              ifelse(del.s.imp >= del.cutoff, 3, NA))),
                                              levels = 1:3,
                                              labels = c('No delirium', '1-2 days delirium',
                                                         '>=3 days delirium')))

## Colors for "stoplight" version
ryg.colors <- c("#00b300", "#F5C11F", "#FF0000")

rbans.spaghetti <- ggplot(#data = subset(rbans.data, del.cat != '1-2 days delirium'),
                          data = rbans.data,
                          aes(x = xval, y = rbans.global.score)) +
  ## Panel by amount of delirium
  facet_wrap(~ del.cat, nrow = 1) +
  ## Add reference zone for normal scores
  annotate(geom = 'rect', fill = 'grey70', alpha = 0.4,
           xmin = -Inf, xmax = Inf, ymin = 100 - 15, ymax = 100 + 15) +
  ## Reference line at normal population mean
  geom_hline(yintercept = 100, colour = 'grey50', size = 1.5) +
  ## Add text/lines denoting normal, impaired zones
  annotate(geom = 'text', label = 'Normal',
           colour = 'grey50', angle = 90, x = 0.3, y = 90, size = 2.5, fontface = 'bold') +
  annotate(geom = 'segment',
           colour = 'grey50',
           arrow = arrow(angle = 30, ends = 'last', type = 'open', length = unit(0.05, 'inches')),
           x = 0.3, xend = 0.3, y = 95, yend = 114) +
  annotate(geom = 'text', label = 'Impaired',
           colour = 'grey50', angle = 90, x = 0.3, y = 79, size = 2.5, fontface = 'bold') +
  annotate(geom = 'segment',
           colour = 'grey50',
           arrow = arrow(angle = 30, ends = 'last', type = 'open', length = unit(0.05, 'inches')),
           x = 0.3, xend = 0.3, y = 73, yend = 46) +
  ## Lines for individual patients + smoothed cohort averages
  geom_line(aes(group = id, colour = del.cat), alpha = 0.15) +
  geom_smooth(aes(colour = del.cat, fill = del.cat), alpha = 0.5, method = 'lm', level = 0.95) +
  scale_x_continuous(name = NULL,
                     breaks = c(1, 12/3, 48/3),
                     labels = c('3M', '12M', '4 Years')) +
  scale_y_continuous(name = NULL,
                     # limits = c(45, 130),
                     breaks = c(45, 60, 78, 85, 100, 115, 130),
                     labels = c('45', '60', '\n78\n(-1.5 SD)', '\n85\n(-1 SD)',
                                '\n100\n(Pop. Mean)', '\n115\n(+1 SD)', '130')) +
  scale_color_viridis(end = 0.75, discrete = TRUE, direction = -1) +
  scale_fill_viridis(end = 0.75, discrete = TRUE, direction = -1) +
  # scale_color_manual(values = ryg.colors) +
  # scale_fill_manual(values = ryg.colors) +
  theme_minimal() +
  theme(#panel.background = element_rect(fill = 'grey98'),
        panel.border = element_rect(color = 'grey80', fill = NA),
        axis.text = element_text(hjust = 0.6, size = 8, face = 'bold'),
        axis.ticks.x = element_blank(),
...
rbans.spaghetti
ggsave(filename = 'brain_rbans_4yr_spaghetti.pdf', device = 'pdf', width = 7, height = 4)

## Mayur really wants a version with no axis labels
rbans.spaghetti +
  theme(axis.text = element_blank())
ggsave(filename = 'brain_rbans_4yr_spaghetti_nolabels.pdf', device = 'pdf', width = 7, height = 4)
