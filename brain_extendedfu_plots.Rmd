...
## Patients >= 65
pts65 <- subset(brain.oneobs, id %in% keepPts & age.enroll >= 65)$id
## Patients >= 75
pts75 <- subset(brain.oneobs, id %in% keepPts & age.enroll >= 75)$id
...
```{r bar_allpts}
bardataAll <- brainStatus %>%
  group_by(timept) %>%
  summarise(propNormal = mean(generalStatus %in% c('No baseline impairment', 'Normal cognition'),
                              na.rm = TRUE),
            propImpaired = mean(generalStatus %in% c('Mild CI at baseline', 'Cognitive impairment'),
                                na.rm = TRUE),
  ...
  mutate(time.f = factor(timept,
                         levels = c(1:3, 5:6),
                         labels = c('In-Hospital', '3 Months', '12 Months', '4 Years', '6 Years')),
...
ggbarAll <- ggplot(data = bardataAll, aes(x = status.f, y = proportion)) +
  ...
  labs(title = 'Status by Time Point among All Patients',
       caption = paste('Proportion of all', length(keepPts),
                       'patients who remained in the study throughout their hospital stay.\n"Cognitive impairment" (CI) is defined by RBANS global score <=78 and/or Trailmaking A or B score <=35.')) +
  # theme_minimal() +
...
```{r bar_knownpts}
bardataKnown <- brainStatus %>%
  filter(generalStatus != 'Unknown') %>%
  group_by(timept) %>%
  summarise(propNormal = mean(generalStatus %in% c('No baseline impairment', 'Normal cognition'),
                              na.rm = TRUE),
            propImpaired = mean(generalStatus %in% c('Mild CI at baseline', 'Cognitive impairment'),
...
ggbarKnown <- ggplot(data = bardataKnown, aes(x = status.f, y = proportion)) +
  facet_wrap(~ time.f, scales = 'free_x', nrow = 1, drop = TRUE) +
  geom_bar(aes(fill = status), stat = 'identity') +
  ...
  scale_y_continuous(name = NULL,
                     ...
                     breaks = seq(0, 1, 0.1),
                     labels = paste0(seq(0, 1, 0.1)*100, '%')) +
  labs(title = 'Status by Time Point among Patients with Known Status',
       caption = paste('Proportion of patients who had complete cognitive assessments or were known to be deceased.\n"Cognitive impairment" (CI) is defined by RBANS global score <=78 and/or Trailmaking A or B score <=35.')) +
  theme(legend.position = 'none',
        strip.text = element_text(face = 'bold'),
        axis.ticks.x = element_blank(),
        axis.text.x = element_text(size = 7.5),
...
bardataAssessed <- brainStatus %>%
  ...
  summarise(propNormal = mean(generalStatus %in% c('No baseline impairment', 'Normal cognition'),
            ...
            propImpaired = mean(generalStatus %in% c('Mild CI at baseline', 'Cognitive impairment'),
                                na.rm = TRUE)) %>%
  gather(key = status, value = proportion, propNormal:propImpaired) %>%
  mutate(time.f = factor(timept,
                         levels = c(1:3, 5:6),
                         labels = c('Enrollment', '3 Months', '12 Months', '4 Years', '6 Years')),
         status.f = factor(ifelse(timept == 1 & status == 'propNormal', 1,
                           ifelse(timept == 1 & status == 'propImpaired', 2,
                           ifelse(status == 'propNormal', 3,
                           ifelse(status == 'propImpaired', 4, NA)))),
                           levels = 1:4,
                           labels = c('No baseline CI',
                                      'Mild CI,\nbaseline',
                                      'No CI',
                                      'CI')))

ggbarAssessed <- ggplot(data = bardataAssessed, aes(x = status.f, y = proportion)) +
  facet_wrap(~ time.f, scales = 'free_x', nrow = 1, drop = TRUE) +
  geom_bar(aes(fill = status), stat = 'identity') +
  ...
  scale_y_continuous(name = NULL,
                     ...
                     breaks = seq(0, 1, 0.1),
                     labels = paste0(seq(0, 1, 0.1)*100, '%')) +
  labs(title = 'Status by Time Point among Patients with Complete Assessments',
       caption = paste('Proportion of patients who had complete cognitive assessments (or survived hospital stay).\n"Cognitive impairment" (CI) is defined by RBANS global score <=78 and/or Trailmaking A or B score <=35.')) +
  theme(legend.position = 'none',
        strip.text = element_text(face = 'bold'),
        axis.ticks.x = element_blank(),
        axis.text.x = element_text(size = 7.5),
